...
int scancommand(string text)
{
	...
	if (commant == 26 || commant == 50)
	{
		...
		if (!rooast.empty())
		{
			...
			struct _stat64 st;
			if ((_wstat64(rooast.c_str(), &st) != 0))
				cout << "Failed root!";
			else
				root = rooast;
			wcout << endl << endl << "Root at         " << root << endl;
		}
	}
	if (commant == 27)
	{
	...
	{
		/*
		WCHAR wchars[MAX_PATH + 12];
		swprintf(wchars, L"dir \"%ls\"", root.c_str());
		_wsystem(wchars);
		//it's old,and it are use CMD.exe
		// i will make a new
		*/
		vector<wstring> fnames;
		getFileNames(root, fnames);
		struct _stat64 st64;
		tm* time;
		unsigned __int64 allbyte = 0;
		unsigned dirs = 0, files = 0;
		stringstream ss;
		wchar_t *_wt = new wchar_t[2048];
		wcout << "Volume in drive " << root[0] << "\nThe directory is:" << root << "\b \n\n";
		for (unsigned long long i = 0; i < fnames.size(); i++)
		{
			_wstat64((root + fnames[i]).c_str(), &st64);
			time = localtime(&st64.st_mtime);//f**k i never know it need add root!!!!
			swprintf
			(
				_wt, L"%04d/%02d/%02d  %02d:%02d:%02d     % 5s % 15ls   %ls",//Prepairs
				time->tm_year + 1900, time->tm_mon + 1, time->tm_mday, time->tm_hour, time->tm_min, time->tm_sec,//time
				bool2wstring((st64.st_mode & 0x4000), L"<DIR>", bool2wstring((st64.st_mode & 0x8000), L"     ", L"<UKN>")).c_str(),
				bool2wstring((st64.st_mode & 0x8000), to_wstring(st64.st_size), L"               ").c_str(),//informations
				fnames[i].c_str()//informations
			);
			...
			if (st64.st_mode & _S_IFDIR)
				...
				files++;
			allbyte += st64.st_size;
			wprintf(L"%ls\n", _wt);
		}
		cout << endl << endl << "\tFile(s):" << files << endl << "\tDirectory(s):" << dirs << endl << "\t" << "All file bytes:" << allbyte << endl;
	}
	if (commant == 35)
	{
		wloadfile();
	...
	if (commant == 59)
	{
		system("start https://github.com/Psertded/PowerSystem/issues");
	}
	if (commant == 60)
	{
		if (this_pin != "NULL")
	...
	if (commant == 68)
	{
		cout << "Type Q to exit calculate" << endl;
		cout << "--------------------------------------------------" << endl;
		wstring str;
		while (wcin >> str && !premium::comcmp(tolower(str), L"Q"))
		{
			cout << premium::calc(str) << '\n';
		}
	}
	if (commant == 69)
	{
		...
		wstring ex;
		getline(wcin, ex);
		ShellExecute(NULL, NULL, fname.c_str(), ex.c_str(), NULL, SW_SHOWNORMAL);
	...
	if (commant == 70)
	{
		...
		ios::sync_with_stdio(false);
		HRESULT get;
		wstring advcode = to_wstring(random(0x0, 0xffffffff));
		if (rellist.GUIDownload == 0)
		{
			DownloadProgress_GCL progress;
			prg.init();
			printf("Press [%c] to exit the download.%c", rellist.downexitkey, '\n');
			IBindStatusCallback* callback = (IBindStatusCallback*)&progress;
			get = URLDownloadToFile(NULL, url.c_str(), (origroot + wstring(L"temp") + wstring(advcode)).c_str(), NULL, static_cast<IBindStatusCallback*>(&progress));
		}
		else if (rellist.GUIDownload == 1)
		{
			DownloadProgress progress;
		...
		IMAGE dstimg;
		loadimage(&dstimg, (origroot + wstring(advcode)).c_str());
#undef initgraph
		initgraph(dstimg.getwidth(), dstimg.getheight());
		putimage(0, 0, &dstimg);
		...
		closegraph();
		_wremove((origroot + L"temp" + advcode).c_str());
	}
	if (commant == 70)
	{
